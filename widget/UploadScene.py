...
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QPushButton, QGridLayout, QComboBox
import cv2
import openpyxl
...
class UploadScene(QWidget):
    def __init__(self, filename):
        super(UploadScene, self).__init__()
        ...
        # 图像显示
        layout = QGridLayout()
        self.useBtn = QPushButton("上传现场照片")
        ...
        # 有无违法行为的判断
        self.legal_label = QLabel("是否有违法行为")
        self.legal_comb = QComboBox()
        self.legal_comb.addItems(["有违法行为", "无违法行为"])
        ...
        layout.addWidget(self.legal_label, 3, 0),
        layout.addWidget(self.legal_comb, 3, 1, 1, 2)
    ...
    def release(self):
        try:
            ...
            print(e)
        if self.legal_comb.currentText() == "有违法行为":
            return False
        else:
            return True
