...
import openpyxl
from PyQt5.QtWidgets import QWidget, QLabel, QGridLayout, QDateTimeEdit, QLineEdit, QPushButton, QTableWidget, \
    QHeaderView, QTableWidgetItem, QComboBox, QMessageBox

from widget.Function import get_yan_info, get_pinpai_and_guige, get_tezheng_info, tezheng1, tezheng2, createLingHe


class LingHeKY(QWidget):
    def __init__(self, base_path):
        super(LingHeKY, self).__init__()
        ...
        layout.addWidget(self.label_location, 5, 0)
        layout.addWidget(self.edit_location, 5, 1)
        # 分类table
        ...
        layout.addWidget(self.table_info, 6, 0, 1, 4)
        # 完成按钮
        self.btn_finish = QPushButton("完成")
        layout.addWidget(self.btn_finish, 7, 0, 1, 4)
        # 设置格式
    ...
    def handle_info(self):
        ...
        # if not weituoren:
        #     QMessageBox.critical(self, "错误", "请填写委托人姓名", QMessageBox.Yes)
        #     return False
        result_data["weituoren"] = weituoren
        shenfenzheng1 = self.edit_shenfenzheng1.text()
        # if not shenfenzheng1:
        #     QMessageBox.critical(self, "错误", "请填写委托人身份证件号", QMessageBox.Yes)
        #     return False
        ...
        # 创建html
        try:
            createLingHe(wb, result_data, self.base_path)
        except Exception as e:
            print(e)
        wb.save(self.filename)
        return True
    ...
    def handle_table_info(self, ws):
        ...
        for index in range(self.table_info.rowCount()):
            self_count += int(self.table_info.item(index, 2).text().split(' ')[0])
            self_total += float(self.table_info.item(index, 3).text())
